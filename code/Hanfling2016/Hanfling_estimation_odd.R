# This File pulls in data from Hanfling et al. 2016 and used the odd numbered communities as 
# mock communities.
...
library(here)
  ...
  #to cross-validate, treat some even mock comm samples as unknown
  ...
  A <- all %>% group_by(community) %>% mutate(tot.conc=sum(start_conc_ng)) %>% ungroup() %>%
          mutate(true_prop = start_conc_ng/tot.conc) %>% dplyr::select(community,Species,true_prop)
  A.wide <- pivot_wider(A,id_cols="Species", names_from="community",values_from="true_prop") %>% as.data.frame()
  
    
  B <-  distinct(community,Species) %>% arrange(community,Species) %>% mutate(val=1)
  ...
  env <- env %>%
    ...
    for(i in 1:nrow(p_mock_small)){
      alr_mock_true_prop_small[i,] <- alr(p_mock_small[i,4:(ncol(p_mock_small))] + 1e-12)
    }
...
save(Output,file=paste0("./data/summarized_data/Hanfling_no_overdisp_odd",".Rdata"))

## Plots
rstan::traceplot(stanMod,pars=c("lp__","alpha[1]","alpha[2]"),inc_warmup=FALSE)

#rstan::traceplot(stanMod,pars=c("eta_mock[5,5]","eta_mock[5,6]","eta_mock[5,7]"),inc_warmup=FALSE)
#rstan::traceplot(stanMod,pars=c("beta[5,5]","beta[5,6]","beta[5,7]"),inc_warmup=FALSE)
#rstan::traceplot(stanMod,pars=c("tau"),inc_warmup=FALSE)
...
#ggsave(p11, filename = "Hanfling_Fish_Mock.pdf", width = 7, height = 5, units = "in")

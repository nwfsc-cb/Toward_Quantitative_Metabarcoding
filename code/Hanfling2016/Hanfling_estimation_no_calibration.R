# This file runs the Hanfling data but does not account for amplification bias.
...
library(here)
  ...
  #to cross-validate, treat some mock comm samples as unknown
   env <- read.csv(here("data/Hanfling2016/Mock_cytB.csv"), stringsAsFactors = F) %>%
  ...
  mc <- read.csv(here("data/Hanfling2016/Mock_cytB.csv"), stringsAsFactors = F) %>%  #mock comm samples
    ...
    sp.list   <- data.frame(Species = sort(unique(env$Species)) ) %>% mutate(sp_idx =1:length(Species))
...
stanMod = stan(file = "./code/Core_Stan_models/quant_metabar_no_overdispersion_no_alpha.stan" ,data = stan_data,
...
save(Output,file=paste0("./data/summarized_data/Hanfling_no_overdisp_no_calibration",".Rdata"))

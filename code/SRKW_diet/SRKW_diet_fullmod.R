...
#Likelihood
# 
# M <- stan_model("./code/quant_metabar_no_overdispersion.stan")
...
#   mutate(id= sort(rep(1:N_species, length(sample_data_small$community)))) %>%
#   mutate(sample_id= rep(sample_data_small$sample_id, N_species)) %>%
#   left_join(., sp.list )
# 
# 
# # merge in predictions to the env data.frame
# ppp <- dat.long %>%
#   left_join(.,estProportions %>% dplyr::select(-sample_id)) %>%
#   #drop_na() %>% 
...
# p1
# 
# BREAKS <- c(0.01,0.05,0.10,0.20,0.30,0.40,0.50,0.75)
# p2.raw <- ggplot(ppp) +
#   geom_point(aes(x= trueProp,y=propReads,color=Species),alpha=0.5) +
#   geom_abline(intercept=0,slope=1,color="red") +
#   scale_y_continuous(limits=c(0,max(BREAKS)),breaks=BREAKS) +
#   scale_x_continuous(limits=c(0,max(BREAKS)),breaks=BREAKS) +
...
# Additional Models:
  # Exclude Mock community samples. Rerun models.

# CLR of alpha among species.
# works on a vector of input values X
alr_to_clr <- function(X){
  val <- exp(X) / sum(exp(X))  
  log_g_mean <-  (1/N_species) * sum(log(val))
  val_clr <- log(val) - log_g_mean
  return(val_clr)
}

clr_raw <- apply(pars$alpha,1,alr_to_clr) %>% t()

PROBS <- c(0.025,0.05,0.25,0.50,0.75,0.95,0.975)
alpha_clr <- data.frame(Mean = colMeans(clr_raw),
                        t(apply(clr_raw,2,quantile,probs=PROBS)))
colnames(alpha_clr)[2 : ncol(alpha_clr)] <- paste0("q.",PROBS)
alpha_clr$id <- 1:N_species
alpha_clr <- left_join(alpha_clr,sp.list)

ggplot(alpha_clr ) +#%>% filter(mock.sp =="Y")) +
    geom_point(aes(x=sp,y=Mean,fill=sp.lab,color=sp.lab),shape=21) +
    geom_errorbar(aes(x=sp,ymin=q.0.05,ymax=q.0.95,color=sp.lab),width=0) +
    scale_fill_viridis_d("Mock\ncommunity",begin=0,end=0.5,option = "plasma")+
    scale_color_viridis_d("Mock\ncommunity",begin=0,end=0.5,option = "plasma")+
    theme_bw() +
    theme(axis.text.x = element_text(angle = 60,hjust=1))
   # Add shape to show species in the mock communities.
...
orca_poo$alpha_clr <- alpha_clr

save(orca_poo, file="./data/summarized_data/kw_diet_output_fullmodel.RData")

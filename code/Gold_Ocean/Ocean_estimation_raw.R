...
ocean_dat <- readRDS("./data/Gold_Ocean/mifish_mock_community_data_North_Ocean.RDS") %>% 
  ...
  # Get rid of North_Skew_2 because of pipeting problem
  #filter(str_detect(community, "North_Skew_2", negate = T)) 
...
sp.by.comm.slim <- sp.by.comm %>%
  dplyr::select(-conc,-tot_conc,-prop) %>%
...
#REQUIRED FORMAT
  ...
  mc <- ocean_dat %>%  
    filter(str_detect(community, "Even", negate = F)) %>%
    filter(str_detect(community, "Coastal", negate = T)) %>%
...
save(Output,file=paste0("./data/summarized_data/Fish_Oceanic_and_North_Raw",".Rdata"))

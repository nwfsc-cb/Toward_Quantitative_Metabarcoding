data { // 
  ...
  //int N_obs_mock_small; // Number of observed mock samples for individual mock comms.

  vector[N_obs_samp] N_pcr_samp ; // vector of PCR cycles used in each sample.
  // vector[N_obs_mock] N_pcr_mock ; // vector of PCR cycles used in each mock community.
  ...
  // vector[N_obs_samp]  log_obs_seq_samp;
  ...
  // vector[N_obs_mock]  log_obs_seq_mock;
...
parameters {
  ...
  // vector[N_e_samp_col] eta_samp;
  // vector[N_e_mock_col] eta_mock;
}

transformed parameters {
  //vector<lower=0>[N_species-1] tau; // vector overdispersion sd for multinomial.
  ...
  // matrix[N_species,N_obs_mock] prob_mock_t;
    ...
    // alpha[1:(N_species-1)] = alpha_prior[1] + alpha_raw * alpha_prior[2]; // non-centered param
    ...
    // for (l in 1:(N_species-1)) {
    //   eta_mock[l] = eta_mock_raw[l] * tau[l] ;
    //   eta_samp[l] = eta_samp_raw[l] * tau[l] ;
    // }

  // from betas, alphas, and etas we can calculate sample-specific mu
    for (n in 1:N_species) {
      logit_val_samp[,n] = model_matrix_b_samp * beta[n]; 
      //                         // eta_samp[n];
...
generated quantities {
  matrix[N_obs_samp_small,N_species] int_samp_small; // estimates, in proportion space
